                                 ...
                                 const struct imxrt_netc_link_s *link)
{
  ...
  putreg32(0x00f00000u, NETC_SWITCH_PORT_BPDVR(port));
  ...
  if (port != 4)
    {
      ...
      if (link->speed == 1000)
        {
          mode |= 1u << NETC_SWITCH_IFMODE_SPEED_SHIFT;
        }
      else if (link->speed == 10)
        {
          mode |= 2u << NETC_SWITCH_IFMODE_SPEED_SHIFT;
        }
      ...
      putreg32(NETC_SWITCH_MAC_TX_ENABLE |
  ...
  const struct imxrt_netc_link_s *port2)
{
  ...
  ninfo("NETC: resetting switch function\n");
  ...
  imxrt_netc_switch_port_configure(0, port0);
  imxrt_netc_switch_port_configure(2, port2);
  imxrt_netc_switch_port_configure(4, &internal);
...
static int imxrt_netc_rings_initialize(struct imxrt_netc_driver_s *priv)
{
  ...
  putreg32(NETC_SI_SIRBGCR_ONE_GROUP, IMXRT_NETC_SI_SIRBGCR);
...
static int imxrt_netc_transmit(struct imxrt_netc_driver_s *priv)
{
  ...
  desc->word1 = (uint64_t)priv->dev.d_len |
                ((uint64_t)priv->dev.d_len << 16) |
                NETC_TX_SWITCH_DIRECT(priv->active_port) |
                NETC_TX_FINAL;
