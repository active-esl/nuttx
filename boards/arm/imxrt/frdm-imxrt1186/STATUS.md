...
| FlexSPI2 / XIP image | Fork-only XIP lane (`nuttx-rt1186-xip`); flash at `0x04000000`, text at `0x0400b000` | **PASS:** LinkServer program @ `0x04000000` | Keep off apache `#19858`; still need clean POR `nsh>` (no gdb load) |
| Boot / J60 SDP | J60 `100` = SDP; `001` = QSPI/POR (NXP/Zephyr FRDM docs) | **Attempted 2026-08-16:** J60→`001` + hard POR; `:2325` quiet (ser2net banner only) | Likely need SDP re-program then `001`+POR when someone is at the bench; soft reset insufficient |
...
| Upstream CI / Make board build | Fork pre-upstream workflow on `active-esl/nuttx`; board has `src/CMakeLists.txt` and `src/Makefile` (tip `f02b92adc111`) | CMake/Ninja `frdm-imxrt1186:nsh` **PASS**; local GNU Make **PASS** (`libboard.a`); checkpatch **PASS**; docs **PASS**. Run [31954904354](https://github.com/active-esl/nuttx/actions/runs/31954904354): Apache Lint **FAIL** (super-linter `bad object` on PR merge `GITHUB_SHA`); cibuild **FAIL** (`testbuild` normalize dirty `defconfig`: drop default `LPUART1_BAUD`/`NSH_DISABLE_IFUPDOWN`, use savedefconfig header) | Push normalized `defconfig` + fix lint `GITHUB_SHA` on fork workflow; re-run gate before any Apache push |
| Upstream PR posture | Apache `#19858` tip tracks FRDM-only CM33 NSH + clocks; fork draft [active-esl/nuttx#4](https://github.com/active-esl/nuttx/pull/4) | Tip `f02b92adc111`; RAM NSH + clocks/ELE/SysTick **PASS** | XIP/eth stay fork-only; no `origin/apache` write without MCQ; no Co-authored-by Cursor; do not ping Michael |
